...
def get_supabase_client() -> Client:
    ...
    url = settings.SUPABASE_URL or (DEFAULT_LOCAL_URL if settings.is_development or settings.is_test else "")
    key = settings.SUPABASE_ANON_KEY or (DEFAULT_LOCAL_KEY if settings.is_development or settings.is_test else "")
    ...
    env_name = "TEST" if settings.is_test else "DEVELOPMENT" if settings.is_development else "PRODUCTION"
    ...
    try:
        ...
        if settings.is_development or settings.is_test:
            try:
                ...
                logger.info(f"Successfully connected to Supabase: {len(response.data)} records returned")
                print(f"Successfully connected to Supabase ({env_name}): {len(response.data)} records returned")
...
def get_supabase_admin_client() -> Client:
    ...
    # Get the Supabase URL and key from environment or use defaults for development
    url = settings.SUPABASE_URL or (DEFAULT_LOCAL_URL if settings.is_development or settings.is_test else "")
    key = settings.SUPABASE_SERVICE_KEY or (DEFAULT_LOCAL_SERVICE_KEY if settings.is_development or settings.is_test else "")
    
    if not url or not key:
        error_msg = "Supabase URL and service key must be provided in production environment"
        logger.error(error_msg)
        if settings.is_production:
            raise ValueError(error_msg)
