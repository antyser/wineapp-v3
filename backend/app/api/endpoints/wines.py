...
async def read_wines(
    ...
    """
    try:
        response = supabase.table("wines").select("*").range(skip, skip + limit).execute()
...
async def read_wine(
    ...
    """
    try:
        response = supabase.table("wines").select("*").eq("id", wine_id).single().execute()
...
async def create_wine(
    wine: WineCreate,
    supabase: Client = Depends(get_supabase_admin)  # Use admin client for wine creation
):
    """
    Create a new wine.
    """
...
async def update_wine(
    ...
    try:
        ...
        check_response = supabase.table("wines").select("id").eq("id", wine_id).execute()
        ...
        response = supabase.table("wines").update(update_data).eq("id", wine_id).execute()
...
async def delete_wine(
    wine_id: str,
    supabase: Client = Depends(get_supabase_admin)  # Use admin client for wine deletion
):
    """
    Delete a wine by ID.
    """
